...
df_tmp <- read_xlsx(paste0(base_dir, "data/swanson_sporns_2020.xlsx"), sheet = 3) %>%
...
df_whs_to_swanson <- read_xlsx(paste0(base_dir, "data/WHS_analysis/WHS_to_Swanson_cortex.xlsx")) # n = 68 (Swanson + WHS combined)
...
load(paste0(analysis_objects_dir, "03June2024_null_nets.RDS")) # df_null_nets (generated in data_analysis/01.validate_MIND/generate_null_nets.R)
...
hippocampal_regions_bm4 <- df_whs_to_bm4 %>% 
  ...
  pull(broad_Swanson_name) %>% 
  unique
parahippocampal_regions_bm4 <- df_whs_to_bm4 %>% 
  left_join(df_system_hierarchy, by = join_by("WHS_name" == "region_of_interest")) %>% 
...
p_tract_tracing <- df_jaccard_mind %>%
  group_by(edge = paste0(pmin(broad_Swanson_name1, broad_Swanson_name2), "-", pmax(broad_Swanson_name1, broad_Swanson_name2))) %>% 
  slice(1) %>% ungroup %>% 
  mutate(color = case_when(
    str_detect(edge, paste0(hippocampal_regions_bm4, collapse = "|")) ~ "Hippocampal",
    str_detect(edge, paste0(parahippocampal_regions_bm4, collapse = "|")) ~ "Parahippocampal",
    str_detect(edge, paste0(paste0(hippocampal_regions_bm4, collapse = "|"), "|", paste0(parahippocampal_regions_bm4, collapse = "|"))) ~ "Both",
    TRUE ~ "neither"
  )
  ) %>% 
  filter(color != "Hippocampal") %>% 
  #mutate(label = ifelse(jaccard < 0.65 & mind_weight > 0.75, edge, "")) %>% 
  ...
  labs(x = "Tract-tracing Jaccard index", y = "Edge weight (w)",
       title = "B | Tract-tracing similarity without HIP edges") +
  theme(
...
hippocampal_regions_amba <- df_whs_to_aba %>% 
  ...
  unique
parahippocampal_regions_amba <- df_whs_to_aba %>% 
  left_join(df_system_hierarchy, by = join_by("WHS" == "region_of_interest")) %>% 
  filter(str_detect(system, "Parahippocampal")) %>% 
  pull(parcellation_structure) %>% 
  unique
...
p_amba <- df_mind_merfish %>%
  ...
  slice(1) %>% ungroup %>% 
  #filter(!(parcellation_structure1 %in% parahippocampal_regions) & !(parcellation_structure2 %in% parahippocampal_regions)) %>% 
  mutate(color = case_when(
    str_detect(edge, paste0(hippocampal_regions_amba, collapse = "|")) ~ "Hippocampal",
    str_detect(edge, paste0(parahippocampal_regions_amba, collapse = "|")) ~ "Parahippocampal",
    str_detect(edge, paste0(paste0(hippocampal_regions_amba, collapse = "|"), "|", paste0(parahippocampal_regions_amba, collapse = "|"))) ~ "Both",
    TRUE ~ "neither"
  )
  ) %>% 
  filter(color != "Hippocampal") %>% 
  mutate(label = ifelse(gene_cor < 0.65 & mind_weight > 0.75, edge, "")) %>% 

  ggplot(aes(x = gene_cor, y = mind_weight)) +
  geom_point(aes(color = color), size = 0.75) +
  #geom_text_repel(aes(label = label), size = 2, max.overlaps = 50) +
