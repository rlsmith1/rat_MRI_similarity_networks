...
length(merfish_roi_order) # n = 46 (with OB regions)


# Combine expression similarity network with MIND weights ------------------

df_mind_merfish_withOB <- df_normative_mind_withOB %>% 
...
load(paste0(analysis_objects_dir, "08July2024_null_nets_withOB.RDS")) # df_null_nets_withOB (generated in data_analysis/01.validate_MIND/withOB/generate_null_nets.R)

## Map atlas labels in null networks and combine with actually gene expression similarity values (by edge)
df_null_nets_mouseTranscriptome <- df_null_nets_withOB %>% 
...
spearman_rho <- cor.test(
  df_mind_merfish_withOB %>% filter(parcellation_structure1 != parcellation_structure2) %>% filter(mind_weight != 0) %>% pull(mind_weight),
  df_mind_merfish_withOB %>% filter(parcellation_structure1 != parcellation_structure2) %>% filter(mind_weight != 0) %>% pull(gene_cor),
  method = "spearman"
) %>% .$estimate
df_null_nets_cor_mind_withOB <- df_null_nets_cor %>% 
  dplyr::select(-data) %>% 
  bind_rows(
    tibble(
...
save(
  df_mind_merfish_withOB, df_null_nets_cor_mind_withOB,
  file = paste0(analysis_objects_dir, "08July2024_gene_expression_withOB.RData")
)
