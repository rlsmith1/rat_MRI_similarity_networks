...
save(df_roi_centers, file = paste0(base_dir, "objects/WHS_centers.RDS"))
...
df_roi_centers %>% 
  ggplot(aes(x = x, y = z)) +
  geom_point(aes(color = paste0(region_of_interest, "_", hemisphere))) +
  theme_void() +
...
df_whs_atlas %>% 
  ...
  #filter(str_detect(region_of_interest, "olf")) %>% 
  
  # plot
  ggplot() +
  geom_polygon(mapping = aes(x = x, y = y, group = region_of_interest, fill = region_of_interest), color = "black", alpha = 0.2) +
  geom_point(
    data = df_roi_centers,
    mapping = aes(x = x, y = y, color = region_of_interest)
  ) +
  geom_text_repel(
    data = df_roi_centers %>% mutate(label = ifelse(y > 750, region_of_interest, "")),
