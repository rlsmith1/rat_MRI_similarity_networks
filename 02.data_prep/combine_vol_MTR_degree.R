...
load(paste0(base_dir, "objects/03June2024_df_vol_cortex.RDS")) # df_vol_cortex (generated in calc_ROI_volume.R)
load(paste0(base_dir, "objects/03June2024_df_mtr_cortex.RDS")) # df_mtr_cortex (generated in calc_ROI_MTR.R)
load(paste0(base_dir, "objects/03June2024_df_degree_cortex.RDS")) # df_strength_cortex (generated in calc_ROI_strength.R)
...
df_vol_cortex %>% 
  ...
  dplyr::select(study, subject, timepoint, scan_date) %>% 
  distinct() %>% 
  filter(!(study == "GSK" & timepoint %in% c(20, 300))) %>% 
...
save(df_data_cortex, file = paste0(base_dir, "objects/03June2024_combined_data_cortex.RDS")) 
