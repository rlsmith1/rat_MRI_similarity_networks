...
df_data %>%
  filter(region_of_interest %in% gray_matter_rois & 
           !(str_detect(region_of_interest, regex('brainstem', ignore_case = TRUE))) &
           !(str_detect(region_of_interest, regex('cerebel', ignore_case = TRUE)))
  ) %>% 
  pull(region_of_interest) %>% unique
