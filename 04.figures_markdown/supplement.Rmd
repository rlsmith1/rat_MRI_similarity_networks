...
figS13a <- ggplot(mapping = aes(x = age, y = value, fill = value, color = value)) +
  ...
  labs(y = "MTR", title = "A | Cortical MTR through \ndevelopment by subject")
```

```{r FigS13a.save, fig.height = 2.5, fig.width = 3}
figS13a
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S13a.development_MTR_by_subj", .x), width = 3, height = 2.5)
)
```
