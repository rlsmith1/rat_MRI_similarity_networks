...
df_node_stress_roi <- df_data_cortex %>%
...
## Set up permutation
n_perm <- 1000
df_node_stress_roi_permAll <- tibble()
...
for (i in 1:n_perm) {
  ...
  df_node_stress_roi_permAll <- df_node_stress_roi_permAll %>% 
    bind_rows(
      df_tmp %>% 
...
save(
  df_node_stress_roi, df_node_stress_roi_perm,
  file = paste0(analysis_objects_dir, "27Sept2024_node_RMS_effects_ROI.RData")
)


# POST-HOC: sex effects ---------------------------------------------------

## RMS
df_node_stress_sexEffects <- df_data_cortex %>%
  filter(study == "GSK" & timepoint == 63) %>% # only looking at the young adult timepoint following RMS
  ...
  mutate(
    model_res = map(
      ...
      .f = ~ f_lm_model(
        dataframe = .x,
        independentVar = "group*sex",
        dependentVar = "value",
        covariates = c("tbv", "age")
      )$results 
    ) 
  ) %>% 
...
## AS
df_node_adultStress_sexEffects <- df_data_cortex %>%
  filter(study == "GSK" & timepoint == 300) %>%
