...
df_edge_adultStress_system <- df_mind_cortex_system %>%
  filter(timepoint == 300) %>% # only looking at the mid adult timepoint following AS
...
df_edge_adultStress_system %>%
  ...
  geom_point(aes(color = group), position = position_jitterdodge(dodge.width = 0.75, jitter.width = 0.2)) +
...
## Set up permutation
n_perm <- 1000
df_edge_adultStress_system_permAll <- tibble()
doParallel::registerDoParallel()
set.seed(0311)
...
for (i in 1:n_perm) {
  ...
  df_edge_adultStress_system_permAll <- df_edge_adultStress_system_permAll %>% 
    bind_rows(
      df_tmp %>% 
...
df_edge_adultStress_system_perm <- df_edge_adultStress_system %>% 
  ungroup %>% 
  filter(term == "groupMS") %>% 
  dplyr::select(system_edge, t_value) %>% 
  mutate(perm = "actual", .before = 1) %>% 
  bind_rows(
      df_edge_adultStress_system_permAll %>% 
      filter(term == "groupMS") %>% 
...
save(
  df_edge_adultStress_system, df_edge_adultStress_system_perm,
  file = paste0(analysis_objects_dir, "11Mar2025_edge_AS_effects_SYS.RData")
)
